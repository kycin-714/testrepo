Play here: https://arcade.makecode.com/S23836-33253-84555-16323 

## Table of contents
* [General info](#general-info)
* [How to play](#how-to-play)
* [Technologies](#technologies)
* [Sources & Inspiration](#sources-and-inspiration)

## General info
_Lantern Rush_ is a single player platformer adventure game. After waking up in the middle of knowhere, you must navigate your way back home and pick up the decorations and items that are necessary for a proper Chinese New Year dinner! Jump through platform courses, dodge the spirits of the evil spirits and find the 6 hidden collectibles long the way.

## How to play
Use W A S D to control the player. Use [space] to jump. Walk over the collectibles to pick up.

## Technologies
Project is created with:
* MacBook Pro
* Microsoft MakeCode Arcade
* Python

## Sources and Inspiration
* How to Make a Platformer Game [Part 2: Animations, AI, collision & spawning] (https://www.youtube.com/watch?v=8DhGcCPfOU4)
* How to Make a Platformer Game [Part 3: Wall jumping, more levels, and fireballs] (https://www.youtube.com/watch?v=CPkC37FzmnU)
* SVG Paths (https://developer.mozilla.org/en-US/docs/Web/SVG/Tutorials/SVG_from_scratch/Paths)
* ...plus various games like Hollow Knight and Dead Souls 


This game was created by Kylar Chan, a university student, for an unexpected coding subject in 2025.
